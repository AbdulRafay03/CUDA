...
int main(){
    ...
    const int outputSize = width * height * outChannels * outChannels * batchSize;
    const int kernelElements = kernelSize * kernelSize * inChannels * outChannels;

    std::cout << "Image Size: " << width << "x" << height << "x"  << inChannels << "x" << outChannels << std::endl;
    ...
    cudnnTensorDescriptor_t kernelDesc;
    ...
    void* d_workspace;
    CHECK_CUDNN(cudaMalloc(&d_workspace,workspaceSize));
    ...
    for(int i = 0; i < benchmarkRuns; i++){
        ...
        naiveConv2d<<<gridSize, blockSize>>>(d_input, d_kernel, d_output_naive, width, height, inChannels, outChannels, kernelSize, batchSize);
        CHECK_CUDA(cudaEventRecord(stop));
        CHECK_CUDA(cudaDeviceSynchronize(stop));
