//A CUDA stream is a queue of GPU work.
// without streams: Copy A → Kernel 1 → Copy B → Kernel 2
// with streams: Stream 1: Copy A → Kernel 1
//               Stream 2: Copy B → Kernel 2
...
int main(void){
    ...
    // make sure d_B is copied to device
    ...
    for (int i = 0; i < numElements; i++){
        if(fabs(h_A[i]+h_B[i] - h_C[i]) > 1e-5) {
            fprintd(stderr, "Results verification failed at element %d", i);
            exit(EXIT_FAILURE);
        }
    }
